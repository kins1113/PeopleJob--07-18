...
CREATE TABLE THIRD (
	...
	THIRDNAME   VARCHAR(100) NULL,     -- 3차직종명
	SECOND_CODE NUMBER       NULL      -- 2차직종코드
);
...
alter table payment rename column patmentway to paymentway;
alter table payment add(jobopening number);
alter table payment add constraint fk_payment foreign key(jobopening)references jobopening(jobopening);

--결제테이블 컬럼 이용기간으로 변경
alter table payment rename column discount to period;
--결제테이블 이용시작날짜컬럼 추가
alter table payment add (paystart_date varchar2(50));
--결제 테이블에 금액 컬럼 추가
alter table payment add (price number);
...
alter table post add admin_code number ; --관리자도 글쓸수 있도록s
alter table post add constraint ADMIN_CODE_FK FOREIGN KEY(ADMIN_CODE)
REFERENCES manager(ADMIN_CODE) ON DELETE CASCADE;   --관리자 코드 fk
alter table post modify member_code null;        --사용자 null 가능 하도록

alter table upload_info modify downcount default 0;     --다운로드 수 기본값
alter table upload_info modify fileName varChar2(200);     --파일명 길이 조절
alter table upload_info modify originalfileName varChar2(200);  --파일명 길이조절
alter table post add report_day DATE default sysdate;    --신고 날짜 추가
alter table post
drop column report_coment;

alter table post 
modify (report_coment varchar2(2000));
ALTER TABLE post add(report_coment varchar2(2000));
...
ALTER TABLE HOPE_WORKING DROP COLUMN JOBTYPE;
ALTER TABLE HOPE_WORKING DROP COLUMN FIRSTJOB_CODE;
ALTER TABLE HOPE_WORKING ADD(BTYPE_CODE3 NUMBER ); --3차업종
ALTER TABLE HOPE_WORKING ADD(THIRD_CODE NUMBER );--3차 직종
...
-- 1차업종
CREATE TABLE BTYPE1 (
	BTYPE_CODE1 NUMBER       PRIMARY KEY, -- 1차업종코드
	BTYPENAME1  VARCHAR(100) NULL      -- 1차업종이름
);
--drop sequence BTYPE1_SEQ;
create sequence BTYPE1_SEQ
increment by 1
start with 1
nocache;
-- 2차업종
CREATE TABLE BTYPE2 (
	BTYPE_CODE2 NUMBER       PRIMARY KEY, -- 2차업종코드
	BTYPENAME2  VARCHAR(100) NULL,     -- 2차업종이름
	BTYPE_CODE1 NUMBER       NULL      -- 1차업종코드
);
--drop sequence BTYPE1_SEQ;
create sequence BTYPE2_SEQ
increment by 1
start with 1
nocache;
-- 3차업종
CREATE TABLE BTYPE3 (
	BTYPE_CODE3 NUMBER       PRIMARY KEY, -- 3차업종코드
	BTYPENAME3  VARCHAR(100) NULL,     -- 3차업종이름
	BTYPE_CODE2 NUMBER       NULL      -- 2차업종코드
);
--drop sequence BTYPE1_SEQ;
create sequence BTYPE3_SEQ
increment by 1
start with 1
nocache;
...
alter table jobopening modify agelimit varchar2(50);
...
ALTER TABLE CERTIFICATE modify L_NAME  null;
ALTER TABLE CERTIFICATE modify L_INSTITUTION  null;
...
start with 1
nocache;
insert into manager(admin_code,adminid,adminpwd,authority,aut_regdate)
values(manager_seq.nextval,'1','1','GM',sysdate);
...
CREATE TABLE LOCATION (
	...
	LOCAL_CODE2 NUMBER      NOT NULL  -- 2차지역코드
	
);
alter table location modify (SIDO null);
alter table location modify (LOCAL_CODE2 null);
CREATE TABLE LOCATION2(
	LOCAL_CODE2 NUMBER      NOT NULL, -- 2차지역코드
	GUGUN       VARCHAR(50) NOT NULL  -- 시군구
	
);
alter table location modify (GUGUN null);
...
alter table EDUCATION add(GRADUATE2 varchar2(50)); 
alter table EDUCATION modify(SCHOOLNAME varchar2(50) NULL); 
alter table EDUCATION modify(MAJOR  NULL);
alter table EDUCATION modify(degree NULL);
...
ALTER TABLE CAREER ADD(WORKTERM2 varchar2(50)); 
commit;
...
-- 2차직종     25
--DROP TABLE SECOND CASCADE CONSTRAINTS;
-- 2차직종
CREATE TABLE SECOND (
	SECOND_CODE NUMBER       PRIMARY KEY, -- 2차직종코드
	SECONDNAME  VARCHAR(100) NULL,     -- 2차직종명
	FIRST_CODE  NUMBER       NULL      -- 1차직종코드
);
...
alter table company rename column COMPANYNO to company_code;
alter table company add (WomanNum varchar2(10)); --여자사원수
alter table company add (ManNum varchar2(10)); --남자사원수
alter table company add (establishYear varchar2(100)); --설립년도
alter table company add (CompanyType varchar2(100)); --기업형태
alter table company add (site varchar2(100)); --홈페이지
alter table company add (sales varchar2(20)); --매출액
alter table company add (capital varchar2(20)); --자본금
alter table company add (majorBusiness varchar2(200)); --주요사업
alter table company add (introduction CLOB); --기업소개
...
-- 1차직종 28
--DROP TABLE FIRST CASCADE CONSTRAINTS;

CREATE TABLE FIRST (
	FIRST_CODE NUMBER      PRIMARY KEY, -- 1차직종코드
	FIRSTNAME  VARCHAR(100) NULL      -- 1차직종명
);
...
alter table scrap drop constraint FK_JOBOPENING_TO_SCRAP;
ALTER TABLE "SCRAP"
   ADD
...
alter table apply drop constraint FK_JOBOPENING_TO_APPLY;
ALTER TABLE "APPLY"
   ADD
...
alter table popup
add START_DAY varchar2(30);
alter table popup
add END_DAY varchar2(30);
SELECT * FROM POPUP;
...
ALTER TABLE "HOPE_WORKING"
	ADD
		CONSTRAINT "FK_THIRD_TO_HOPE_WORKING" -- 3차직종 -> 희망근무조건
		FOREIGN KEY (
			"THIRD_CODE" -- 3차직종코드
		)
...
ALTER TABLE "HOPE_WORKING"
	ADD
		CONSTRAINT "FK_BTYPE3_TO_HOPE_WORKING" -- 3차업종 -> 희망근무조건
		FOREIGN KEY (
			"BTYPE_CODE3" -- 3차업종코드
		)
		REFERENCES "BTYPE3" ( -- 3차업종
			"BTYPE_CODE3" -- 3차업종코드
		);
-- 지원현황
ALTER TABLE "APPLY"
...
ALTER TABLE "THIRD"
	ADD
		CONSTRAINT "FK_SECOND_TO_THIRD" -- 2차직종 -> 3차직종
		FOREIGN KEY (
			"SECOND_CODE" -- 2차직종코드
		)
...
ALTER TABLE "SECOND"
	ADD
		CONSTRAINT "FK_FIRST_TO_SECOND" -- 1차직종 -> 2차직종
		FOREIGN KEY (
			"FIRST_CODE" -- 1차직종코드
		)
...
ALTER TABLE "LOCATION"
	ADD
		CONSTRAINT "FK_LOCATION2_TO_LOCATION" -- 지역2 -> 지역
		FOREIGN KEY (
			"LOCAL_CODE2" -- 2차지역코드
		)
		REFERENCES "LOCATION2" ( -- 지역2
			"LOCAL_CODE2" -- 2차지역코드
		);
...
ALTER TABLE "BTYPE3"
	ADD
		CONSTRAINT "FK_BTYPE2_TO_BTYPE3" -- 2차업종 -> 3차업종
		FOREIGN KEY (
			"BTYPE_CODE2" -- 2차업종코드
		)
		REFERENCES  "BTYPE2" ( -- 2차업종
			"BTYPE_CODE2" -- 2차업종코드
		);
-- 2차업종
ALTER TABLE "BTYPE2"
	ADD
		CONSTRAINT "FK_BTYPE1_TO_BTYPE2" -- 1차업종 -> 2차업종
		FOREIGN KEY (
			"BTYPE_CODE1" -- 1차업종코드
		)
		REFERENCES "BTYPE1" ( -- 1차업종
			"BTYPE_CODE1" -- 1차업종코드
		);
