...
<script>
	...
	$(document).ready(function(){
		...
		$("#del").click(function(){
			var cnt=0;
			if(confirm("삭제하시겠습니까?")){
				$("input[name=custextCode1]").each(function(){
					if($(this).is(':checked')==true){
						cnt=1;
					}
				})
				if(cnt==0){
					alert("삭제할 문의사항을 체크해주세요.");
					event.preventDefault();
					return false;
				}else{
					location.href="<c:url value='/custext/custextmultiDel.do?custextCode='/>"+$('#custextCode').val();
				}
			}
		});
		$("#custext").click(function(){
			location.href="<c:url value='/custext/insertcustext.do'/>";
		})
	});
	function s_it()
	{
	  var total_str = "";
	  var obj = document.getElementsByName("custextCode1");
	  ...
	  document.getElementById("custextCode").value = total_str;
	}
</script>
<style type="text/css">
.divList {
...
<%-- 		<div id="where">
	  ...
	  <!-- 반복문 시작  -->	  	
					<tr>
					<!-- 제목이 긴 경우 30글자만 보여주기 -->
					<td><input type="checkbox" name="custextCode1" id="custextCode1"value="${vo.custextCode }" onclick="javascript_:s_it()"></td>
					<td>
					<a href="<c:url value='/custext/custext_view.do?custextCode=${vo.custextCode}'/>">
					${vo.custitle}
					</a></td>
	   				<td><fmt:formatDate value="${vo.cusRegdate}" pattern="yyyy-MM-dd"/></td>
	   				<td>
	    			...
	    			<c:if test="${vo.answerRegdate!=null}">
	    			<%-- ${vo.answerRegdate } --%>
					<fmt:formatDate value="${vo.answerRegdate}" pattern="yyyy-MM-dd"/>	    			
	    			</c:if>
	    			</td>
				</tr>
	    	</c:forEach>
  ...
  <!--반복처리 끝  -->	
	...
	<input type="hidden" name="custextCode" id="custextCode"> 
	<div class="pull-right">
      <input type="button" id="custext" name="custext" class="btn btn-primary" role="button" value="문의하기">
